...
| PANEL-09 | In progress | `077acfd` | `Scripts/test_run_local_ui_tests.rb`: 5/5 passed (53 assertions); `zsh -n` passed; project regeneration and repeatability verification passed; signed `build-for-testing` produced distinct validated app and runner bundles | Real `test-without-building` suite awaits exclusive UI-lock availability | Baseline runner was unsigned and killed before bootstrap; repaired build produced app SHA-256 `5fd7bb8cd5654561a5ae0b35fbdedabe4ca88f690762ca04ef0b03dee799216a` and runner SHA-256 `0a71f97576f62c5712f4000c11f081bfd5a739c97c220deb9a5cc2a7eea7ae11` | Build/sign/identity separation is verified, but PANEL-09 is not fixed until the real NSPanel/SwiftUI UI suite connects and passes under the lock. |
...
| `077acfd` | PANEL-09 target-specific macOS app/test identities plus a signed two-stage Local UI runner guarded by the exclusive UI lock | Runner tests passed 5/5 (53 assertions); project generation/verification passed; signed isolated app and XCTest runner built, validated, and were hashed. Live suite execution remains pending. |
