...
actor AttachmentFileStore {
    ...
    func importFiles(
        ...
        guard existingCount + urls.count <= AttachmentLimits.maxAttachmentsPerNote else {
    ...
    func materializedURL(for reference: AttachmentFileReference) throws -> URL {
        ...
        return directory.appendingPathComponent(reference.filename, isDirectory: false)
    }
    ...
    func ensureMaterialized(_ reference: AttachmentFileReference) throws -> URL? {
        ...
        if fileManager.fileExists(atPath: url.path),
           ...
           SHA256.hash(data: existing).hexString == reference.digest,
           reference.byteCount == nil || Int64(existing.count) == reference.byteCount {
            return url
        }
    ...
    /// Reconciles only the canonical app-owned roots. Missing files are
    /// recreated from valid payloads; missing payloads never delete model rows.
    func reconcile(_ references: [AttachmentFileReference]) throws {
        try prepare()
        let expected = Set(references.map {
    ...
    private func importOne(
        ...
        guard expectedByteCount <= AttachmentLimits.maxBytesPerAttachment else {
        ...
        #if os(macOS) || os(iOS)
        startedSecurityScope = sourceURL.startAccessingSecurityScopedResource()
        #endif
        defer {
            #if os(macOS) || os(iOS)
    ...
    static func sanitizedFilename(_ filename: String) -> String {
        ...
        let fallback = value.isEmpty ? "attachment" : value
